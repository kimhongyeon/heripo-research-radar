...
export const parseJinjuMuseumList = (html: string): ParsedTargetListItem[] => {
  ...
  $('table tbody tr').each((index, element) => {
    ...
    const fullUrl = new URL(
      `/kor/html/sub06/0601.html${relativeHref}`,
      baseUrl,
    );
...
export const parseJinjuMuseumDetail = (html: string): ParsedTargetDetail => {
  const $ = cheerio.load(html);
