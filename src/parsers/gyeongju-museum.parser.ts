...
export const parseGyeongjuMuseumList = (
  html: string,
  hrefPrefix: string,
...
export const parseGyeongjuMuseumNoticeList = (
  html: string,
): ParsedTargetListItem[] => {
  const $ = cheerio.load(html);
  const posts: ParsedTargetListItem[] = [];
  ...
  $('table tbody tr').each((index, element) => {
    ...
    const fullUrl = new URL(
      `/kor/html/sub07/0703.html${relativeHref}`,
      baseUrl,
    );
    const detailUrl = fullUrl.href;
    const uniqId = fullUrl.searchParams.get('no') ?? undefined;
...
export const parseGyeongjuMuseumDetail = (html: string): ParsedTargetDetail => {
  const $ = cheerio.load(html);
